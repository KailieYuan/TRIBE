...
document.addEventListener('DOMContentLoaded', function(e){
		...
		parent.innerHTML += "<span>NEWS</span>"; 
		parent.innerHTML += "<span>RUNWAY</span>";
		parent.innerHTML += "<span>SPINOUTS</span>";
	}
    ...
    $(window).scroll(function(){
		...
		var newsOff = $("#news").offset().top-100;
		var fillOff = $("#filterstick").offset().top-50;
		var runwayOff = $("#runway").offset().top-80;
		var spinoutsOff = $("#spinouts").offset().top-80;

		//News Section
			if(scrollPos < fillOff){
...
function getValidUsers() {
    let arr = [];
    let a = new User('Admin', 'admin@gmail.com', 'adminpw', 'admin', false);
...
function isValidUser(uEmail, uPassword){
    ...
    if(valid){
    	...
    	document.getElementById("correctSign").classList.addClass('hidden');
}
		...
		if(data.length){
			data.forEach(function (item) {
				if(item.id == index){
					...
					container.find('h4').text(item.project.description);
					container.find('#emailcompany').text(item.project.contactemail);
					container.find('#webcompany').text(item.project.websiteurl);
	...
	function renderFilterResults(filters, students){
		...
		var criteria = ['company-category','studio-professor-1'],
			results = [],
			isFiltered = false;
